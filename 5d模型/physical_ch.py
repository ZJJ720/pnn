...
def E_con(T, I, Q, A_e, C_3, C_4):
    E_con_p = torch.empty(len(I))
    E_con_n = torch.empty(len(I))

    for i in range(len(I)):
        E_con_p[i] = -R*T[i]/F*torch.log(1-I[i]/(1.43*math.pow(10, -4)*F*math.pow(Q[i]/A_e[i], 0.4)*C_4[i]))
        E_con_n[i] = -R*T[i]/F*torch.log(1-I[i]/(1.43*math.pow(10, -4)*F*math.pow(Q[i]/A_e[i], 0.4)*C_3[i]))
...
def E_act(T, I, S, k_p, k_n, A_e, C_2, C_3, C_4, C_5):
    ...
    E_act = R*T*2/F*(torch.asinh(I/S/w_e*A_e/2/F/k_p/torch.sqrt(C_4*C_5)) + torch.asinh(I/S/w_e*A_e/2/F/k_n/torch.sqrt(C_2*C_3)))
