...
def E_con(T, I, Q, A_e, C_2, C_5):
    E_con_p = torch.empty(len(I))
    E_con_n = torch.empty(len(I))

    for i in range(len(I)):
        E_con_p[i] = -R*T[i]/F*torch.log(1-I[i]/(1.43*math.pow(10, -4)*F*math.pow(Q[i]/A_e[i], 0.4)*C_5[i]))
        E_con_n[i] = -R*T[i]/F*torch.log(1-I[i]/(1.43*math.pow(10, -4)*F*math.pow(Q[i]/A_e[i], 0.4)*C_2[i]))
